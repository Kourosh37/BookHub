...
export async function bookingsExport(req: Request) {
  try {
    ...
    const header = ["عنوان برنامه", "نام رزروکننده", "شماره رزروکننده", "تاریخ", "ساعت", "پاسخ ها"];
    ...
    if (format === "xls" || format === "excel") {
      ...
      return new NextResponse(html, { headers: { "Content-Type": "application/vnd.ms-excel; charset=utf-8", "Content-Disposition": `attachment; filename=\"${fileName}\"` } });
    }
...
export async function cancelBooking(req: Request, bookingId: string) {
  ...
  try {
    ...
    if (!booking) return NextResponse.json({ error: "رزرو پیدا نشد" }, { status: 404 });
    const isHost = booking.schedule.userId === session.userId;
    const isGuest = booking.bookedByUserId === session.userId;
    if (!isHost && !isGuest) return NextResponse.json({ error: "عدم دسترسی", details: "اجازه کنسل این رزرو را ندارید" }, { status: 403 });
