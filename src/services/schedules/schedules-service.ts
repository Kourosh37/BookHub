...
export async function schedulesCreate(req: Request) {
  ...
  try {
    ...
    for (const day of normalizedDays) if (day.date < todayInTehran) return NextResponse.json({ error: `تاریخ ${day.date} گذشته است`, details: "تاریخ برنامه نباید قبل از تاریخ امروز باشد" }, { status: 400 });
    ...
    if (!slots.length) return NextResponse.json({ error: "بازه قابل رزرو تولید نشد", details: "بازه‌های واردشده با مدت جلسه سازگار نیست یا خیلی کوتاه است." }, { status: 400 });
    ...
    if (conflicting) {
      ...
      return NextResponse.json({ error: "تداخل زمانی با جلسات موجود", details: hit ? `بازه انتخابی با ${describeConflict(hit)} همپوشانی دارد.` : "بازه انتخابی با جلسات موجود همپوشانی دارد." }, { status: 409 });
    }
    const schedule = await prisma.schedule.create({
      data: { shareId: createShareId(), userId: session.userId, title: parsed.data.title, questions: parsed.data.questions, daysConfig: normalizedDays, slotDuration: parsed.data.slotDuration, gapMinutes: parsed.data.gapMinutes },
    });
    ...
    if (!session) return NextResponse.json({ error: "عدم دسترسی" }, { status: 401 });
    return NextResponse.json({ error: "خطا در ساخت برنامه", details: error?.message || "خطای ناشناخته در پردازش فرم" }, { status: 500 });
  }
}
...
export async function scheduleUpdateTitle(req: Request, scheduleId: string) {
  try {
    ...
    if (!parsed.success) {
      const firstIssue = parsed.error.issues[0];
      return NextResponse.json({ error: "داده نامعتبر است", details: firstIssue?.message || "Invalid payload" }, { status: 400 });
    }
...
export async function scheduleBook(req: Request, shareId: string) {
  ...
  if (!slot) return NextResponse.json({ error: "بازه زمانی پیدا نشد", details: "لینک یا بازه انتخابی معتبر نیست" }, { status: 404 });
  const now = new Date();
  if (slot.endTime <= now || slot.startTime <= now) return NextResponse.json({ error: "بازه زمانی منقضی شده", details: "امکان رزرو بازه‌های گذشته یا شروع‌شده وجود ندارد" }, { status: 400 });
  if (slot.isBooked) return NextResponse.json({ error: "این بازه همین الان رزرو شد", details: "لطفاً یک بازه دیگر انتخاب کنید" }, { status: 409 });
  ...
  if (conflict) return NextResponse.json({ error: "تداخل زمانی با جلسات موجود", details: `این بازه با ${describeConflict(conflict)} همپوشانی دارد.` }, { status: 409 });
  const existing = await prisma.booking.findFirst({ where: { scheduleId: schedule.id, bookedByUserId: session.userId }, select: { id: true } });
  if (existing) return NextResponse.json({ error: "قبلا رزرو انجام شده است", details: "هر کاربر برای هر برنامه فقط یک نوبت می‌تواند رزرو کند" }, { status: 409 });
  try {
    ...
    if (bookingDetails) {
      ...
      const ctx = { bookingId: bookingDetails.id, scheduleId: bookingDetails.scheduleId, scheduleTitle: bookingDetails.schedule?.title, hostUserId: bookingDetails.schedule.userId, guestUserId: bookingDetails.bookedByUserId, guestName, slotStartIso: bookingDetails.timeSlot?.startTime?.toISOString?.() || null };
    ...
    if (e instanceof Error && e.message === "SLOT_TAKEN") return NextResponse.json({ error: "این بازه همین الان رزرو شد", details: "لطفاً یک بازه دیگر انتخاب کنید" }, { status: 409 });
